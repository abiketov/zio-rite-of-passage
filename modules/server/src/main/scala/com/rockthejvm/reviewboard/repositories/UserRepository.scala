...
class UserRepositoryLive private (quill: Quill.Postgres[SnakeCase]) extends UserRepository {
  ...
  inline given addMeta: InsertMeta[User]    = insertMeta[User](_.id)
  inline given changeMeta: UpdateMeta[User] = updateMeta[User](_.id)

  override def create(user: User): Task[User] =
    run(
      query[User].insertValue(lift(user)).returning{r =>
       r
      }
    )

  override def getById(id: Long): Task[Option[User]] =
    run(query[User].filter(u => u.id == lift(id))).map(_.headOption)

  override def getByEmail(email: String): Task[Option[User]] =
    run(query[User].filter(u => u.email == lift(email))).map(_.headOption)
